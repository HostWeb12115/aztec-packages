...
We welcome Barretenberg hackers! Barretenberg is unaudited code. Currently, we are most interested in feedback on UX and auditing the protocol.

- [Barretenberg](#barretenberg-development)
...
"Why is barretenberg in C++?" is often asked as there is significant momentum for Rust in ZK. This is primarily a historic decision.
There are pros and cons of C++:
Pros: C++ compilers are very mature and robust. C++'s has a rich feature-set that is geared towards abstractions that do not slow down the underlying operations. C++ has a very mature and robust suite of tooling.
Cons: When using zero cost abstractions to their fullest, a significant downside is that you expose a lot of C++'s complexity - and have to be careful to not bog down compile time.

We are currently content continuing in C++. We encourage anyone looking to port the prover or solidity verifier to wait until they are both audited and stable.
...
- Install llvm with `brew install llvm@20`
