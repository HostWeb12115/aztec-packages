...
//  * @details Construct a stdlib proof/verification_key for each entry in the native verification queue. By default,
//  both
//  * are constructed from their counterpart in the native queue. Alternatively, Stdlib verification keys can be
//  provided
//  * directly as input to this method. (The later option is used, for example, when constructing recursive verifiers
//  based
...
//     // Fiat-Shamir the accumulator. (Only needs to be performed on the first in a series of recursive PG
//     verifications
...
//  * @details The recursive verifier will be either Oink or Protogalaxy depending on the specified proof type. In
//  either
...
//  */
// std::tuple<std::shared_ptr<ClientIVC::RecursiveVerifierInstance>, ClientIVC::PairingPoints,
// ClientIVC::TableCommitments> ClientIVC::perform_recursive_verification_and_databus_consistency_checks(
//     ClientCircuit& circuit,
...
//         // Add randomness at the end of the hiding kernel (whose ecc ops fall right at the end of the op queue table)
//         to
...
//     // cannot affect the ECC-op accumulator for the kernel. For the tail kernel, we additionally add a preceeding
//     no-op
...
//         // Add randomness at the begining of the tail kernel (whose ecc ops fall at the beginning of the op queue
//         table)
...
//  * initialize the folding accumulator. Otherwise, execute the PG prover to fold the prover instance into the
//  accumulator
...
//     // verifiers for all the entries previously present in the verification queue) and if it's not the first
//     accumulate
...
//     // Update native verifier accumulator and construct merge proof (excluded for hiding kernel since PG terminates
//     with
...
//  * doesn't reveal information about actual ecc operations in the transaction, when the proof is sent to the rollup,
//  we
...
//  * being ClientIVC. ECCVM is not affected by the concerns above, randomness being added to wires at proving time as
//  per
...
//  *
//  * We need each op queue polynomial to have 9 random coefficients (so the op queue needs to contain 5 random ops,
//  every
//  * UltraOp adding two coefficients to each of the 4 polynomials).
//  *
//  * For the last subtable of ecc ops belonging to the hiding kernel, merged via appended to the full op queue, its
//  data
...
//  * commitment to the `left_table_inverse` plus its evaluation at Merge protocol challenge κ. Also for the degree
//  check,
//  * prover needs to send the evaluation of the `left_table` at κ⁻¹. We need to ensure random coefficients are added to
//  * one of the kernels as not to affect Apps verification keys so the best choice is to add them to the beginning of
//  the
...
//  * queue, its data appears as the ecc_op_wires in the MegaZK proof, wires that are not going to be shifted, so the
//  proof
//  * containts, for each wire, its commitment and evaluation to the Sumcheck challenge. As at least 3 random
//  coefficients
