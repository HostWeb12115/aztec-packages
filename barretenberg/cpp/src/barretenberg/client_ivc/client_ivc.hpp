...
//  * operations. It is assumed that the circuits being accumulated correspond alternatingly to an app and a kernel, as
//  is
//  * the case in Aztec. Two recursive folding verifiers are appended to each kernel (except the first one) to verify
//  the
//  * folding of a previous kernel and an app/function circuit. Due to this structure it is enforced that the total
//  number
...
//     /**
//      * @brief A full proof for the IVC scheme containing a Mega proof showing correctness of the hiding circuit
//      (which
...
//          * @details This method results in a buffer that is double-size-prefixed with the buffer size. This is to
//          mimmic
...
using ClientIVC = SumcheckClientIVC;
