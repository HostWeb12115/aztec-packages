...
TEST(ECCVMBoolsRelation, EnforcesBooleanConstraintsOnAllPolynomials)
{
    ...
    // Test 1: The relation should be satisfiable (there exist valid boolean assignments)
    ASSERT_TRUE(s.check()) << "The boolean relation should be satisfiable";

    // Test 2: For each boolean polynomial, verify it must be 0 or 1
    ...
    // For each boolean polynomial, test that it cannot be assigned a non-boolean value
    for (const auto& poly_name : boolean_poly_names) {
        ...
        ASSERT_FALSE(s.check()) << "Polynomial " << poly_name << " should only be able to equal 0 or 1";
        s.pop();
    }
}
