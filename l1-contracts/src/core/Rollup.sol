...
contract Rollup is IStaking, IValidatorSelection, IRollup, RollupCore {
    ...
    )
  {
    ...
    return (
      tips.provenCheckpointNumber,
      rollupStore.archives[tips.provenCheckpointNumber],
      tips.pendingCheckpointNumber,
      rollupStore.archives[tips.pendingCheckpointNumber],
      archiveAt(_myHeaderCheckpointNumber),
      getEpochForCheckpoint(tips.provenCheckpointNumber)
    );
  }
  ...
  function archive() external view override(IRollup) returns (bytes32) {
    RollupStore storage rollupStore = STFLib.getStorage();
    return rollupStore.archives[rollupStore.tips.getPendingCheckpointNumber()];
  }

  function getProvenCheckpointNumber() external view override(IRollup) returns (uint256) {
    return STFLib.getStorage().tips.getProvenCheckpointNumber();
  }

  function getPendingCheckpointNumber() external view override(IRollup) returns (uint256) {
    return STFLib.getStorage().tips.getPendingCheckpointNumber();
  }
  ...
  function getCurrentBlobCommitmentsHash() external view override(IRollup) returns (bytes32) {
    RollupStore storage rollupStore = STFLib.getStorage();
    return STFLib.getBlobCommitmentsHash(rollupStore.tips.getPendingCheckpointNumber());
  }
  ...
  function getFeeAssetPerEth() public view override(IRollup) returns (FeeAssetPerEthE9) {
    return FeeLib.getFeeAssetPerEthAtCheckpoint(STFLib.getStorage().tips.getPendingCheckpointNumber());
  }
  ...
  function archiveAt(uint256 _checkpointNumber) public view override(IRollup) returns (bytes32) {
    RollupStore storage rollupStore = STFLib.getStorage();
    return _checkpointNumber <= rollupStore.tips.getPendingCheckpointNumber()
      ? rollupStore.archives[_checkpointNumber]
      : bytes32(0);
  }
