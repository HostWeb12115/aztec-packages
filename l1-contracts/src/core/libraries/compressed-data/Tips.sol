...
library ChainTipsLib {
  ...
  function getPendingCheckpointNumber(CompressedChainTips _compressedChainTips) internal pure returns (uint256) {
    return CompressedChainTips.unwrap(_compressedChainTips) >> 128;
  }

  function getProvenCheckpointNumber(CompressedChainTips _compressedChainTips) internal pure returns (uint256) {
  ...
  function compress(ChainTips memory _chainTips) internal pure returns (CompressedChainTips) {
    ...
    uint256 pending = _chainTips.pendingCheckpointNumber.toUint128();
    uint256 proven = _chainTips.provenCheckpointNumber.toUint128();
  ...
  function decompress(CompressedChainTips _compressedChainTips) internal pure returns (ChainTips memory) {
    return ChainTips({
      pendingCheckpointNumber: getPendingCheckpointNumber(_compressedChainTips),
      provenCheckpointNumber: getProvenCheckpointNumber(_compressedChainTips)
    });
  }
}
