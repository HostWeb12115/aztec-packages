...
library EpochProofLib {
  ...
  function submitEpochRootProof(SubmitEpochRootProofArgs calldata _args) internal {
    ...
    rollupStore.tips =
      rollupStore.tips.updateProvenCheckpointNumber(Math.max(rollupStore.tips.getProvenCheckpointNumber(), _args.end));
  ...
  function assertAcceptable(uint256 _start, uint256 _end) private view returns (Epoch) {
    ...
    bool isStartBuildingOnProven = _start - 1 <= rollupStore.tips.getProvenCheckpointNumber();
