...
library InvalidateLib {
  ...
  function _validateInvalidationInputs(
    ...
    require(
      _checkpointNumber <= rollupStore.tips.getPendingCheckpointNumber(), Errors.Rollup__CheckpointNotInPendingChain()
    );

    // But not yet proven
    require(_checkpointNumber > rollupStore.tips.getProvenCheckpointNumber(), Errors.Rollup__CheckpointAlreadyProven());
  ...
  function _invalidateCheckpoint(uint256 _checkpointNumber) private {
    RollupStore storage rollupStore = STFLib.getStorage();
    rollupStore.tips = rollupStore.tips.updatePendingCheckpointNumber(_checkpointNumber - 1);
