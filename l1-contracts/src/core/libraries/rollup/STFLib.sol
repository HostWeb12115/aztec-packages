...
library STFLib {
  ...
  function addTempCheckpointLog(TempCheckpointLog memory _tempCheckpointLog) internal {
    uint256 checkpointNumber = STFLib.getStorage().tips.getPendingCheckpointNumber();
  ...
  function prune() internal {
    ...
    uint256 pending = tips.getPendingCheckpointNumber();
    ...
    uint256 proven = tips.getProvenCheckpointNumber();
    rollupStore.tips = tips.updatePendingCheckpointNumber(proven);
  ...
  {
    uint256 pending = getStorage().tips.getPendingCheckpointNumber();
    uint256 size = roundaboutSize();
  ...
  function getEffectivePendingCheckpointNumber(Timestamp _timestamp) internal view returns (uint256) {
    RollupStore storage rollupStore = STFLib.getStorage();
    CompressedChainTips tips = rollupStore.tips;
    return STFLib.canPruneAtTime(_timestamp) ? tips.getProvenCheckpointNumber() : tips.getPendingCheckpointNumber();
  }
  ...
  function getEpochForCheckpoint(uint256 _checkpointNumber) internal view returns (Epoch) {
    RollupStore storage rollupStore = STFLib.getStorage();
    require(
      _checkpointNumber <= rollupStore.tips.getPendingCheckpointNumber(),
      Errors.Rollup__InvalidCheckpointNumber(rollupStore.tips.getPendingCheckpointNumber(), _checkpointNumber)
    );
  ...
  function canPruneAtTime(Timestamp _ts) internal view returns (bool) {
    ...
    if (tips.getPendingCheckpointNumber() == tips.getProvenCheckpointNumber()) {
      return false;
    }

    Epoch oldestPendingEpoch = getEpochForCheckpoint(tips.getProvenCheckpointNumber() + 1);
