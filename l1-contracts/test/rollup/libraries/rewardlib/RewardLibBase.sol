...
contract RewardLibBase is TestBase {
  ...
  modifier prepare(uint96 _blockReward, uint32 _sequencerBps) {
    blockReward = uint96(_bound(_blockReward, 0, 10_000e18));
    sequencerBps = uint32(_bound(_sequencerBps, 0, 10_000));
    feeAsset = IERC20(address(new TestERC20("test", "TEST", address(this))));
    wrapper = new RewardLibWrapper(feeAsset, blockReward, sequencerBps);
    feePortal = wrapper.feePortal();

    deal(address(feeAsset), address(wrapper.rewardDistributor()), blockReward * 300);
    deal(address(feeAsset), address(wrapper.feePortal()), blockReward * 100);
