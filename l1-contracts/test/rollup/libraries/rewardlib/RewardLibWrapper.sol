...
contract RewardLibWrapper {
  ...
  constructor(IERC20 _feeAsset, uint96 _blockReward, uint32 _sequencerBps) {
  ...
  function addFeeHeader(FeeHeader memory _feeHeader) external {
    // Increase the pending block number.
    RollupStore storage rollupStore = STFLib.getStorage();
    CompressedChainTips tips = rollupStore.tips;
    rollupStore.tips = tips.updatePendingBlockNumber(tips.getPendingBlockNumber() + 1);
