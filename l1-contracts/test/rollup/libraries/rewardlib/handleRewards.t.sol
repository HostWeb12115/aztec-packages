...
contract HandleRewardsTest is RewardLibBase {
  ...
  function test_GivenCallerEQCanonicalRollup(uint96 _blockReward, uint32 _sequencerBps)
    external
    prepare(_blockReward, _sequencerBps)
    givenProverHasNotSubmitted
    whenLengthGTLongestProven
  {
    // it store the prover shares
    ...
    uint256 blockCount = 32;

    args.args.proverId = makeAddr("prover2");
    args.end = args.start + blockCount - 1;
    args.fees = _fees(blockCount, sequencer);
    ...
    uint256 totalRewards =
      Math.min(blockReward * (blockCount - 1), feeAsset.balanceOf(address(wrapper.rewardDistributor())));
    ...
    uint256 sequencerRewardsPerBlock = sequencerRewards / (blockCount - 1);
    uint256 proverRewards = totalRewards - sequencerRewards; // no dust
    uint256 dust = sequencerRewards - (sequencerRewardsPerBlock * (blockCount - 1));

    uint256 size = 3;

    if (sequencerRewardsPerBlock > 0) {
      size += (blockCount - 1); // the first was already updated, this is 30 repeat writes for 100 gas each.
    }
    ...
    assertGt(wrapper.getProverShares(Epoch.wrap(0), blockCount, args.args.proverId), 0, "prover shares");
    assertGt(wrapper.getSummedShares(Epoch.wrap(0), blockCount), 0, "summed shares");
    assertEq(wrapper.getLongestProvenLength(Epoch.wrap(0)), blockCount, "longest proven length");

    assertEq(
      wrapper.getSequencerRewards(sequencer),
      initialSequencerRewards + sequencerRewardsPerBlock * (blockCount - 1),
      "sequencer rewards"
    );
    assertEq(
