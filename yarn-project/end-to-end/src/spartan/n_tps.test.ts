import { type SentTx, waitForProven } from '@aztec/aztec.js/contracts';
  ...
  it('can send n_tps', async () => {
    ...
    for (let i = 0; i < sentTxs.length; i++) {
      ...
      try {
        ...
        logger.info(`tx ${i + 1} included in block ${receipt.blockNumber}`);
        results.push({ success: true, tx: sentTx });
      } catch (error) {
        logger.error(`tx ${i + 1} was not included: ${error}`);
    ...
    expect(sentTxs.length).toBe(TOTAL_TXS);
